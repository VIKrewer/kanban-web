...
function DroppableColumn({
  ...
  onCreateTask: (columnId: string, taskData: CreateTaskData) => Promise<void>;
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const taskData = {
      ...
      priority:
        (formData.get("priority") as "baixo" | "medio" | "alto") || "medio",
    };
...
export default function BoardPage() {
  ...
  const lastMoveRef = useRef<{ taskId: string; targetColumnId: string } | null>(
    null,
  );
  ...
  async function createTask(columnId: string, taskData: CreateTaskData) {
    ...
    if (sourceColumn.id === targetColumn.id) {
      ...
      if (activeIndex !== -1 && overIndex !== -1 && activeIndex !== overIndex) {
        setColumns((prev: ColumnWithTasks[]) => {
          ...
          const col = newColumns.find((c) => c.id === sourceColumn.id)!;
      ...
      setColumns((prev: ColumnWithTasks[]) => {
        ...
        const src = newColumns.find((col) => col.id === sourceColumn.id)!;
        const dst = newColumns.find((col) => col.id === targetColumn.id)!;
        ...
        <Dialog open={isEditingTitle} onOpenChange={setIsEditingTitle}>
          <DialogContent className="w-[95vw] max-w-106.25 mx-auto">
            ...
            <form className="space-y-4" onSubmit={handleUpdateBoard}>
              <div className="space-y-2">
                <Label htmlFor="boardTitle">Titulo do Board</Label>
                <Input
                  value={newTitle}
        ...
        <Dialog open={isFilterOpen} onOpenChange={setIsFilterOpen}>
          <DialogContent className="w-[95vw] max-w-106.25 mx-auto">
            <DialogHeader>
              ...
              <p className="text-sm text-gray-600">
                Filtre as tarefas pela prioridade, responsável ou prazo
              </p>
            </DialogHeader>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 space-y-4 sm:space-y-0">
            <div className="flex flex-wrap items-center gap-4 sm:gap-6">
              <div className="text-sm text-gray-600">
                <span className="font-medium">Tarefas totais:</span>
